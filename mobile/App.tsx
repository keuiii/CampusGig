...
type OrderHistoryItem = { id: string; fromStatus: string | null; toStatus: string; note: string | null; createdAt: string; actor: { id: string; displayName: string } };
type OrderFile = { id: string; purpose: string; originalName: string; mimeType: string; sizeBytes: number; createdAt: string };
type OrderRevision = { id: string; sequenceNumber: number; instructions: string; status: string; createdAt: string; resolvedAt: string | null };
type OrderReview = { id: string; overallRating: number; comment: string | null; createdAt: string };
type MobileOrderDetail = MobileOrder & { revisionsUsed: number; revisionLimit: number; files: OrderFile[]; revisions: OrderRevision[]; review: OrderReview | null; history: OrderHistoryItem[] };
type MessageAttachment = { id: string; originalName: string; mimeType: string; sizeBytes: number; createdAt: string };
type OrderMessage = { id: string; body: string | null; createdAt: string; isMine: boolean; attachments: MessageAttachment[]; sender: { id: string; displayName: string; hasAvatar: boolean; avatarVersion: number } };
type MobileNotification = { id: string; type: string; title: string; body: string; orderId: string | null; readAt: string | null; createdAt: string };
...
const DOWNLOAD_DIRECTORY_KEY = "campusgig.downloadDirectory";
...
export default function App() {
  ...
  async function openOrderFromNotification(orderId: string) {
    ...
    if(existing){setSelectedOrder(existing);changeTab("orders");return;}
    const token=await AsyncStorage.getItem(TOKEN_KEY);
    const response=await fetch(`${API_URL}/api/v1/orders/${orderId}`,{headers:{Authorization:`Bearer ${token}`}});
    if(response.ok){const payload=await response.json();setSelectedOrder(payload.data);changeTab("orders");}
  }
...
function Home({ nightMode, user, avatarVersion, query, setQuery, categories, selectedCategory, setSelectedCategory, schools, selectedSchoolId, setSelectedSchoolId, services, loading, refreshing, onRefresh, onOpenService, onOpenProfile, onOpenOrder }: { nightMode: boolean; user: AuthUser; avatarVersion: number; query: string; setQuery: (value: string) => void; categories: Category[]; selectedCategory: string | null; setSelectedCategory: (value: string | null) => void; schools: School[]; selectedSchoolId: string; setSelectedSchoolId: (value: string) => void; services: Service[]; loading: boolean; refreshing: boolean; onRefresh: () => void; onOpenService: (service: Service) => void; onOpenProfile: () => void; onOpenOrder: (orderId: string) => Promise<void> }) {
  ...
  async function openNotification(item:MobileNotification){const token=await AsyncStorage.getItem(TOKEN_KEY);if(!item.readAt){await fetch(`${API_URL}/api/v1/notifications/${item.id}/read`,{method:"PATCH",headers:{Authorization:`Bearer ${token}`}});setUnreadCount((count)=>Math.max(0,count-1));setNotifications((items)=>items.map((entry)=>entry.id===item.id?{...entry,readAt:new Date().toISOString()}:entry));}setNotificationsOpen(false);if(item.orderId)await onOpenOrder(item.orderId);}
  useEffect(() => { void loadNotifications(); const timer=setInterval(()=>void loadNotifications(),8000); return()=>clearInterval(timer); }, [user.id]);
  ...
  </ScrollView><Modal visible={notificationsOpen} transparent animationType="fade" onRequestClose={()=>setNotificationsOpen(false)}><Pressable style={styles.notificationModalBackdrop} onPress={()=>setNotificationsOpen(false)}><Pressable style={[styles.mobileNotificationSheet,nightMode&&darkStyles.card]} onPress={()=>undefined}><View style={styles.mobileNotificationHead}><View><Text style={styles.eyebrow}>NOTIFICATIONS</Text><Text style={[styles.mobileNotificationTitle,nightMode&&darkStyles.primaryText]}>Account activity</Text></View><Pressable onPress={()=>setNotificationsOpen(false)}><Text style={[styles.closeButtonText,nightMode&&darkStyles.primaryText]}>×</Text></Pressable></View>{notifications.length?notifications.slice(0,12).map((item)=><Pressable key={item.id} onPress={()=>void openNotification(item)} style={[styles.mobileNotificationRow,nightMode&&darkStyles.outline]}><View style={[styles.mobileNotificationIcon,!item.readAt&&styles.mobileNotificationIconUnread]}><Text>{item.type==="NEW_MESSAGE"?"✉":item.type==="ORDER_ACCEPTED"?"✓":"↗"}</Text></View><View style={{flex:1}}><Text style={[styles.mobileNotificationRowTitle,nightMode&&darkStyles.primaryText]}>{item.title}</Text><Text style={[styles.mobileNotificationRowBody,nightMode&&darkStyles.mutedText]}>{item.body}</Text><Text style={[styles.mobileNotificationRowTime,nightMode&&darkStyles.mutedText]}>{new Date(item.createdAt).toLocaleString()}</Text></View>{!item.readAt&&<View style={styles.mobileUnreadDot}/>}</Pressable>):<View style={styles.mobileNotificationEmpty}><Text style={[styles.emptyTitle,nightMode&&darkStyles.primaryText]}>No notifications yet</Text><Text style={[styles.emptyBody,nightMode&&darkStyles.mutedText]}>Order updates and messages will appear here.</Text></View>}</Pressable></Pressable></Modal></>;
}
...
function OrderWorkspaceModal({ nightMode, order, onClose }: { nightMode: boolean; order: MobileOrder | null; onClose: () => void }) {
  ...
  const [messageFiles,setMessageFiles]=useState<DocumentPicker.DocumentPickerAsset[]>([]);
  ...
  async function loadWorkspace(showLoading=false) {
    ...
    try {
      ...
      const [detailResponse,messageResponse]=await Promise.all([fetch(`${API_URL}/api/v1/orders/${order.id}`,{headers}),fetch(`${API_URL}/api/v1/orders/${order.id}/messages`,{headers})]);
      if (detailResponse.ok) { const next=(await detailResponse.json()).data as MobileOrderDetail; setDetail(next);setHistory(next.history??[]); }
      if (messageResponse.ok) setMessages((await messageResponse.json()).data??[]);
    } finally { if (showLoading) setLoading(false); }
  }
  useEffect(()=>{
    if (!order) return;
    setDetail(null);setHistory([]);setMessages([]);setDraft("");setMessageFiles([]);setRevisionInstructions("");setReviewComment("");setRating(5);void loadWorkspace(true);
    const timer=setInterval(()=>void loadWorkspace(),5000);
    return()=>clearInterval(timer);
  },[order?.id]);
  async function send() {
    const body=draft.trim(); if(!order||(!body&&!messageFiles.length)||sending)return;
    setSending(true);
    try {
      const token=await AsyncStorage.getItem(TOKEN_KEY);
      let response:Response;
      if(messageFiles.length){const form=new FormData();if(body)form.append("body",body);messageFiles.forEach((file)=>form.append("files",{uri:file.uri,name:file.name,type:file.mimeType??"application/octet-stream"} as unknown as Blob));response=await fetch(`${API_URL}/api/v1/orders/${order.id}/messages/attachments`,{method:"POST",headers:{Authorization:`Bearer ${token}`},body:form});}
      else response=await fetch(`${API_URL}/api/v1/orders/${order.id}/messages`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${token}`},body:JSON.stringify({body})});
      const payload=await response.json().catch(()=>null);
      if(!response.ok)throw new Error(payload?.message??"Unable to send message");
      setMessages((items)=>[...items,payload.data]);setDraft("");setMessageFiles([]);
    } catch(error){Alert.alert("Message not sent",error instanceof Error?error.message:"Please try again.");}
    finally{setSending(false);}
  }
  async function chooseMessageFiles(){const result=await DocumentPicker.getDocumentAsync({type:["image/jpeg","image/png","image/webp","application/pdf","application/zip","application/vnd.openxmlformats-officedocument.wordprocessingml.document","application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"],copyToCacheDirectory:true,multiple:true});if(result.canceled)return;const selected=result.assets.slice(0,3);if(selected.some((file)=>(file.size??0)>15*1024*1024))return Alert.alert("File is too large","Each attachment must be 15 MB or smaller.");setMessageFiles(selected);}
  async function orderAction(path:string,body?:object){if(!order||acting)return;setActing(true);try{const token=await AsyncStorage.getItem(TOKEN_KEY);const response=await fetch(`${API_URL}/api/v1/orders/${order.id}/${path}`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${token}`},body:body?JSON.stringify(body):undefined});const payload=await response.json().catch(()=>null);if(!response.ok)throw new Error(Array.isArray(payload?.message)?payload.message.join(" "):payload?.message??"Unable to update order");Alert.alert("Order updated",payload.message);setRevisionInstructions("");await loadWorkspace();}catch(error){Alert.alert("Unable to continue",error instanceof Error?error.message:"Please try again.");}finally{setActing(false);}}
  async function openDelivery(file:Pick<OrderFile,"id"|"originalName"|"mimeType">){
    ...
    try{
      ...
      const result=await FileSystem.downloadAsync(`${API_URL}/api/v1/orders/${order.id}/files/${file.id}`,temporaryPath,{headers:{Authorization:`Bearer ${token}`}});
      ...
      if(Platform.OS==="android"){
        ...
        if(!directoryUri){
          ...
          if(!permission.granted){Alert.alert("Download cancelled","Choose the Downloads folder so CampusGig can save this delivery.");return;}
          directoryUri=permission.directoryUri;
          await AsyncStorage.setItem(DOWNLOAD_DIRECTORY_KEY,directoryUri);
        }
        try{
          const savedUri=await FileSystem.StorageAccessFramework.createFileAsync(directoryUri,safeName,file.mimeType);
          const base64=await FileSystem.readAsStringAsync(result.uri,{encoding:FileSystem.EncodingType.Base64});
          await FileSystem.writeAsStringAsync(savedUri,base64,{encoding:FileSystem.EncodingType.Base64});
          Alert.alert("Download complete",`${file.originalName} was saved to your selected Downloads folder. You can open it there before accepting or requesting a revision.`);
        }catch{
          await AsyncStorage.removeItem(DOWNLOAD_DIRECTORY_KEY);
          throw new Error("CampusGig could not access the selected folder. Tap the file again and choose Downloads.");
        }
      }else if(await Sharing.isAvailableAsync()){
        await Sharing.shareAsync(result.uri,{dialogTitle:`Save or open ${file.originalName}`,mimeType:file.mimeType});
      }else Alert.alert("File downloaded",result.uri);
    }catch(error){Alert.alert("Unable to download file",error instanceof Error?error.message:"Please try again.");}
    finally{setDownloadingFileId(null);}
  }
  const status=detail?.status??order?.status??"";
  return <Modal visible={Boolean(order)} animationType="slide" onRequestClose={onClose}><SafeAreaView style={[styles.workspaceSafe,nightMode&&darkStyles.surface]}><StatusBar style={nightMode?"light":"dark"}/><KeyboardAvoidingView style={{flex:1}} behavior={Platform.OS==="ios"?"padding":undefined}><View style={[styles.workspaceHeader,nightMode&&darkStyles.outline]}><View style={{flex:1}}><Text style={styles.eyebrow}>ORDER WORKSPACE</Text><Text style={[styles.workspaceTitle,nightMode&&darkStyles.primaryText]} numberOfLines={1}>{order?.title}</Text><Text style={[styles.workspaceOrderNumber,nightMode&&darkStyles.mutedText]}>{order?.orderNumber} · {status.replaceAll("_"," ")}</Text></View><Pressable onPress={onClose} style={[styles.closeButton,nightMode&&darkStyles.card]}><Text style={[styles.closeButtonText,nightMode&&darkStyles.primaryText]}>×</Text></Pressable></View>{loading?<ActivityIndicator color={green} style={{marginTop:50}}/>:<ScrollView contentContainerStyle={styles.workspaceContent} keyboardShouldPersistTaps="handled"><View style={[styles.workspaceSummary,nightMode&&darkStyles.card]}><Text style={[styles.workspaceSectionTitle,nightMode&&darkStyles.primaryText]}>Project requirements</Text><Text style={[styles.workspaceBody,nightMode&&darkStyles.mutedText]}>{order?.requirements}</Text><View style={styles.workspaceFacts}><Text style={[styles.workspaceFact,nightMode&&darkStyles.primaryText]}>₱{((order?.totalCentavos??0)/100).toLocaleString()}</Text><Text style={[styles.workspaceFact,nightMode&&darkStyles.primaryText]}>{order?.package.name}</Text><Text style={[styles.workspaceFact,nightMode&&darkStyles.primaryText]}>Revisions {detail?.revisionsUsed??0}/{detail?.revisionLimit??order?.package.revisionLimit??0}</Text></View></View>{detail?.files?.length?<><Text style={styles.groupLabel}>DELIVERY FILES</Text><View style={[styles.timelineCard,nightMode&&darkStyles.card]}>{detail.files.map((file)=><Pressable disabled={Boolean(downloadingFileId)} onPress={()=>void openDelivery(file)} key={file.id} style={({pressed})=>[styles.deliveryFileRow,pressed&&styles.pressed]}><View style={styles.deliveryFileIcon}>{downloadingFileId===file.id?<ActivityIndicator size="small" color={green}/>:<Text>⇩</Text>}</View><View style={{flex:1}}><Text style={[styles.timelineStatus,nightMode&&darkStyles.primaryText]}>{file.originalName}</Text><Text style={[styles.timelineNote,nightMode&&darkStyles.mutedText]}>{(file.sizeBytes/1024/1024).toFixed(2)} MB · Tap to open · {file.purpose.replaceAll("_"," ")}</Text></View></Pressable>)}</View></>:null}{status==="SUBMITTED"&&<><Text style={styles.groupLabel}>REVIEW THIS DELIVERY</Text><View style={[styles.clientDecisionCard,nightMode&&darkStyles.card]}><Text style={[styles.workspaceSectionTitle,nightMode&&darkStyles.primaryText]}>Is the work ready?</Text><Text style={[styles.workspaceBody,nightMode&&darkStyles.mutedText]}>Accept the delivery to complete the order, or send clear revision instructions.</Text><Pressable disabled={acting} onPress={()=>void orderAction("complete")} style={styles.acceptDeliveryButton}><Text style={styles.authButtonText}>✓ Accept delivery</Text></Pressable>{(detail?.revisionsUsed??0)<(detail?.revisionLimit??0)&&<><TextInput value={revisionInstructions} onChangeText={setRevisionInstructions} multiline maxLength={2000} placeholder="Describe exactly what needs to be changed…" placeholderTextColor="#929A96" style={[styles.revisionInput,nightMode&&darkStyles.input,nightMode&&darkStyles.primaryText]}/><Pressable disabled={acting||revisionInstructions.trim().length<10} onPress={()=>void orderAction("revision",{instructions:revisionInstructions.trim()})} style={[styles.revisionButton,(acting||revisionInstructions.trim().length<10)&&styles.authButtonDisabled]}><Text style={styles.revisionButtonText}>Request revision</Text></Pressable></>}</View></>}{status==="COMPLETED"&&!detail?.review&&<><Text style={styles.groupLabel}>RATE YOUR PROVIDER</Text><View style={[styles.clientDecisionCard,nightMode&&darkStyles.card]}><View style={styles.ratingRow}>{[1,2,3,4,5].map((value)=><Pressable key={value} onPress={()=>setRating(value)}><Text style={[styles.ratingStar,value<=rating&&styles.ratingStarActive]}>★</Text></Pressable>)}</View><TextInput value={reviewComment} onChangeText={setReviewComment} multiline maxLength={1000} placeholder="Share your experience (optional)…" placeholderTextColor="#929A96" style={[styles.revisionInput,nightMode&&darkStyles.input,nightMode&&darkStyles.primaryText]}/><Pressable disabled={acting} onPress={()=>void orderAction("review",{overallRating:rating,qualityRating:rating,communicationRating:rating,timelinessRating:rating,comment:reviewComment.trim()||undefined})} style={styles.acceptDeliveryButton}><Text style={styles.authButtonText}>Submit {rating}-star review</Text></Pressable></View></>}{detail?.review&&<View style={[styles.reviewComplete,nightMode&&darkStyles.card]}><Text style={styles.ratingStarActive}>{"★".repeat(detail.review.overallRating)}</Text><Text style={[styles.workspaceBody,nightMode&&darkStyles.mutedText]}>Your review has been submitted. {detail.review.comment}</Text></View>}<Text style={styles.groupLabel}>ORDER TIMELINE</Text><View style={[styles.timelineCard,nightMode&&darkStyles.card]}>{history.map((item,index)=><View key={item.id} style={styles.timelineRow}><View style={styles.timelineRail}><View style={styles.timelineDot}/>{index<history.length-1&&<View style={[styles.timelineLine,nightMode&&darkStyles.outline]}/>}</View><View style={styles.timelineCopy}><Text style={[styles.timelineStatus,nightMode&&darkStyles.primaryText]}>{item.toStatus.replaceAll("_"," ")}</Text><Text style={[styles.timelineNote,nightMode&&darkStyles.mutedText]}>{item.note} · {item.actor.displayName}</Text><Text style={[styles.timelineDate,nightMode&&darkStyles.mutedText]}>{new Date(item.createdAt).toLocaleString()}</Text></View></View>)}</View><Text style={styles.groupLabel}>ORDER MESSAGES</Text><View style={[styles.messageCard,nightMode&&darkStyles.card]}><ScrollView nestedScrollEnabled showsVerticalScrollIndicator contentContainerStyle={styles.mobileMessageScrollContent} style={styles.mobileMessageScroll}>{messages.length?messages.map((message)=><View key={message.id} style={[styles.messageRow,message.isMine&&styles.messageRowMine]}>{!message.isMine&&<View style={styles.messageAvatar}><Text style={styles.messageAvatarText}>{message.sender.displayName.charAt(0).toUpperCase()}</Text></View>}<View style={[styles.messageBubble,message.isMine?styles.messageBubbleMine:(nightMode?darkStyles.input:styles.messageBubbleOther)]}><Text style={[styles.messageSender,message.isMine&&styles.messageSenderMine]}>{message.isMine?"You":message.sender.displayName}</Text>{message.body&&<Text style={[styles.messageBody,message.isMine&&styles.messageBodyMine,nightMode&&!message.isMine&&darkStyles.primaryText]}>{message.body}</Text>}{message.attachments?.map((file)=><Pressable key={file.id} onPress={()=>void openDelivery(file)} style={styles.mobileChatAttachment}><Text style={[styles.mobileChatAttachmentName,message.isMine&&styles.messageBodyMine]}>⇩ {file.originalName}</Text><Text style={[styles.mobileChatAttachmentMeta,message.isMine&&styles.messageTimeMine]}>{(file.sizeBytes/1024/1024).toFixed(2)} MB · Tap to download</Text></Pressable>)}<Text style={[styles.messageTime,message.isMine&&styles.messageTimeMine]}>{new Date(message.createdAt).toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"})}</Text></View>{message.isMine&&<View style={[styles.messageAvatar,styles.messageAvatarMine]}><Text style={[styles.messageAvatarText,styles.messageAvatarTextMine]}>Y</Text></View>}</View>):<View style={styles.messageEmptyState}><View style={styles.messageEmptyIcon}><Text>✉</Text></View><Text style={[styles.messageEmptyTitle,nightMode&&darkStyles.primaryText]}>Start the conversation</Text><Text style={[styles.messageEmpty,nightMode&&darkStyles.mutedText]}>Messages and reference files stay private to this order.</Text></View>}</ScrollView><View style={[styles.composerArea,nightMode&&darkStyles.navigation]}>{messageFiles.length>0&&<View style={styles.mobileFilePreview}><Text numberOfLines={1} style={styles.mobileFilePreviewText}>{messageFiles.map((file)=>file.name).join(" · ")}</Text><Pressable onPress={()=>setMessageFiles([])}><Text style={styles.mobileFilePreviewClear}>Clear</Text></Pressable></View>}<View style={[styles.composer,nightMode&&darkStyles.navigation]}><Pressable onPress={()=>void chooseMessageFiles()} style={[styles.composerAttach,nightMode&&darkStyles.outline]}><Text style={styles.composerAttachText}>＋</Text>{messageFiles.length>0&&<View style={styles.composerAttachBadge}><Text style={styles.composerAttachBadgeText}>{messageFiles.length}</Text></View>}</Pressable><TextInput value={draft} onChangeText={setDraft} placeholder={messageFiles.length?`${messageFiles.length} file(s) selected · Add a note…`:"Message your provider…"} placeholderTextColor="#929A96" multiline maxLength={2000} style={[styles.composerInput,nightMode&&darkStyles.input,nightMode&&darkStyles.primaryText]}/><Pressable disabled={(!draft.trim()&&!messageFiles.length)||sending} onPress={send} style={[styles.composerSend,((!draft.trim()&&!messageFiles.length)||sending)&&styles.authButtonDisabled]}>{sending?<ActivityIndicator color="#FFF"/>:<Text style={styles.composerSendText}>➤</Text>}</Pressable></View></View></View></ScrollView>}</KeyboardAvoidingView></SafeAreaView></Modal>;
}
...
const styles=StyleSheet.create({
  ...
  openWorkspaceText:{color:green,fontSize:9,fontWeight:"900",marginTop:14,textAlign:"right"},workspaceSafe:{flex:1,backgroundColor:"#FFF",paddingTop:NativeStatusBar.currentHeight??0},workspaceHeader:{paddingHorizontal:20,paddingVertical:14,borderBottomWidth:1,borderBottomColor:"#E5EAE7",flexDirection:"row",alignItems:"center",gap:12},workspaceTitle:{color:ink,fontSize:19,fontWeight:"900",marginTop:4},workspaceOrderNumber:{color:muted,fontSize:9,marginTop:4},workspaceContent:{padding:20,paddingBottom:30},workspaceSummary:{borderWidth:1,borderColor:"#E0E6E2",borderRadius:16,padding:16,backgroundColor:"#F9FBFA"},workspaceSectionTitle:{color:ink,fontSize:14,fontWeight:"900"},workspaceBody:{color:muted,fontSize:11,lineHeight:17,marginTop:8},workspaceFacts:{flexDirection:"row",flexWrap:"wrap",gap:8,marginTop:14},workspaceFact:{color:ink,fontSize:9,fontWeight:"800",paddingHorizontal:10,paddingVertical:7,borderRadius:12,backgroundColor:"rgba(15,107,79,.08)"},timelineCard:{borderWidth:1,borderColor:"#E0E6E2",borderRadius:16,padding:16,backgroundColor:"#FFF"},timelineRow:{flexDirection:"row",gap:12,minHeight:66},timelineRail:{width:16,alignItems:"center"},timelineDot:{width:11,height:11,borderRadius:6,backgroundColor:green,borderWidth:2,borderColor:"#D9F36A"},timelineLine:{width:1,flex:1,backgroundColor:"#DDE4DF",marginVertical:3},timelineCopy:{flex:1,paddingBottom:15},timelineStatus:{color:ink,fontSize:10,fontWeight:"900"},timelineNote:{color:muted,fontSize:9,lineHeight:14,marginTop:4},timelineDate:{color:muted,fontSize:8,marginTop:4},messageCard:{borderWidth:1,borderColor:"#E0E6E2",borderRadius:18,padding:14,minHeight:190,backgroundColor:"#F7FAF8",overflow:"hidden"},messageRow:{flexDirection:"row",alignItems:"flex-end",gap:8,marginVertical:6},messageRowMine:{justifyContent:"flex-end"},messageBubble:{maxWidth:"76%",borderRadius:17,paddingHorizontal:13,paddingVertical:10},messageBubbleOther:{backgroundColor:"#FFF",borderWidth:1,borderColor:"#E1E7E3",borderBottomLeftRadius:5},messageBubbleMine:{backgroundColor:green,borderBottomRightRadius:5,shadowColor:green,shadowOpacity:.16,shadowRadius:7,shadowOffset:{width:0,height:3},elevation:2},messageSender:{color:green,fontSize:8,fontWeight:"900",marginBottom:4},messageSenderMine:{color:"#D9F36A"},messageBody:{color:ink,fontSize:11,lineHeight:16},messageBodyMine:{color:"#FFF"},messageTime:{color:muted,fontSize:7,marginTop:5,textAlign:"right"},messageTimeMine:{color:"#C8DDD4"},messageEmpty:{color:muted,fontSize:9,lineHeight:15,textAlign:"center",marginTop:5},composer:{padding:10,backgroundColor:"#FFF",flexDirection:"row",alignItems:"flex-end",gap:9},composerInput:{flex:1,maxHeight:100,minHeight:44,borderWidth:1,borderColor:"#DCE3DF",borderRadius:14,paddingHorizontal:13,paddingVertical:11,color:ink,backgroundColor:"#F8FAF9",fontSize:11},composerSend:{width:44,height:44,borderRadius:22,backgroundColor:green,alignItems:"center",justifyContent:"center"},composerSendText:{color:"#FFF",fontSize:17,fontWeight:"900"},
  deliveryFileRow:{flexDirection:"row",alignItems:"center",gap:11,paddingVertical:9,borderBottomWidth:1,borderBottomColor:"#E8EDE9"},deliveryFileIcon:{width:35,height:35,borderRadius:10,backgroundColor:"#E5F1EA",alignItems:"center",justifyContent:"center"},clientDecisionCard:{borderWidth:1,borderColor:"#DCE4DF",borderRadius:16,padding:16,backgroundColor:"#FFF"},acceptDeliveryButton:{minHeight:46,borderRadius:11,backgroundColor:green,alignItems:"center",justifyContent:"center",marginTop:14},revisionInput:{minHeight:92,borderWidth:1,borderColor:"#DCE3DF",borderRadius:11,padding:12,textAlignVertical:"top",color:ink,backgroundColor:"#F8FAF9",fontSize:11,lineHeight:16,marginTop:14},revisionButton:{minHeight:43,borderRadius:11,borderWidth:1,borderColor:green,alignItems:"center",justifyContent:"center",marginTop:9},revisionButtonText:{color:green,fontSize:10,fontWeight:"900"},ratingRow:{flexDirection:"row",justifyContent:"center",gap:8,marginBottom:5},ratingStar:{fontSize:31,color:"#D6DDD9"},ratingStarActive:{fontSize:18,color:"#E4A719",fontWeight:"900"},reviewComplete:{borderWidth:1,borderColor:"#DCE4DF",borderRadius:16,padding:16,backgroundColor:"#FFF",marginTop:20},
  mobileChatAttachment:{marginTop:7,padding:9,borderWidth:1,borderColor:"rgba(15,107,79,.22)",borderRadius:10,backgroundColor:"rgba(255,255,255,.12)"},mobileChatAttachmentName:{color:ink,fontSize:9,fontWeight:"900"},mobileChatAttachmentMeta:{color:muted,fontSize:7,marginTop:4},composerAttach:{width:44,height:44,borderWidth:1,borderColor:"#DCE3DF",borderRadius:14,alignItems:"center",justifyContent:"center",position:"relative"},composerAttachText:{color:green,fontSize:24,lineHeight:26},composerAttachBadge:{position:"absolute",right:-5,top:-5,width:17,height:17,borderRadius:9,backgroundColor:green,alignItems:"center",justifyContent:"center"},composerAttachBadgeText:{color:"#FFF",fontSize:7,fontWeight:"900"},
  messageAvatar:{width:29,height:29,borderRadius:10,backgroundColor:"#DDECE4",alignItems:"center",justifyContent:"center",marginBottom:2},messageAvatarMine:{backgroundColor:green},messageAvatarText:{color:green,fontSize:9,fontWeight:"900"},messageAvatarTextMine:{color:"#FFF"},messageEmptyState:{alignItems:"center",paddingVertical:28,paddingHorizontal:20},messageEmptyIcon:{width:44,height:44,borderRadius:15,backgroundColor:"#E5F1EA",alignItems:"center",justifyContent:"center",marginBottom:11},messageEmptyTitle:{color:ink,fontSize:11,fontWeight:"900"},composerArea:{backgroundColor:"#FFF",borderTopWidth:1,borderTopColor:"#E1E6E3",marginHorizontal:-14,marginBottom:-14,marginTop:10},mobileFilePreview:{marginHorizontal:10,marginTop:9,paddingHorizontal:11,paddingVertical:8,borderRadius:9,backgroundColor:"rgba(15,107,79,.09)",flexDirection:"row",alignItems:"center",gap:10},mobileFilePreviewText:{color:green,fontSize:8,fontWeight:"800",flex:1},mobileFilePreviewClear:{color:green,fontSize:8,fontWeight:"900"},
  mobileMessageScroll:{maxHeight:390},mobileMessageScrollContent:{paddingVertical:2,flexGrow:1,justifyContent:"flex-end"},
  notificationModalBackdrop:{flex:1,backgroundColor:"rgba(5,14,10,.58)",padding:18,justifyContent:"center"},mobileNotificationSheet:{maxHeight:"78%",borderRadius:22,backgroundColor:"#FFF",overflow:"hidden",borderWidth:1,borderColor:"#DCE3DF"},mobileNotificationHead:{padding:18,flexDirection:"row",alignItems:"center",justifyContent:"space-between",borderBottomWidth:1,borderBottomColor:"#E5EAE7"},mobileNotificationTitle:{color:ink,fontSize:18,fontWeight:"900",marginTop:4},mobileNotificationRow:{padding:14,borderBottomWidth:1,borderBottomColor:"#E9EDEB",flexDirection:"row",gap:11,alignItems:"flex-start"},mobileNotificationIcon:{width:36,height:36,borderRadius:11,backgroundColor:"#EEF2EF",alignItems:"center",justifyContent:"center"},mobileNotificationIconUnread:{backgroundColor:"#DFF0E7"},mobileNotificationRowTitle:{color:ink,fontSize:11,fontWeight:"900"},mobileNotificationRowBody:{color:muted,fontSize:9,lineHeight:14,marginTop:3},mobileNotificationRowTime:{color:muted,fontSize:7,marginTop:5},mobileUnreadDot:{width:7,height:7,borderRadius:4,backgroundColor:green,marginTop:4},mobileNotificationEmpty:{padding:38,alignItems:"center"},
});
