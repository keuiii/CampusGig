...
type Service = { id: string; title: string; description: string; category: string; provider: string; schoolId: string; school: string; price: number; rating: number; delivery: string; packages: ServicePackage[]; coverMediaId?: string | null; portfolio?: { id: string; originalName: string; mimeType: string }[] };
...
export default function App() {
  ...
  return <SafeAreaView style={styles.safe}><StatusBar style="dark"/><View style={styles.app}><Animated.View style={[styles.animatedScreen,{opacity:screenMotion,transform:[{translateY:screenMotion.interpolate({inputRange:[0,1],outputRange:[8,0]})}]}]}>
    {tab === "home" && (
    ...
    {tab === "orders" && <OrdersScreen orders={orders} loading={ordersLoading} onRefresh={loadOrders} onBrowse={() => changeTab("home")}/>}
  ...
  return <SafeAreaView style={styles.authSafe}>
    ...
    <KeyboardAvoidingView style={styles.authKeyboard} behavior={Platform.OS === "ios" ? "padding" : undefined}>
      <ScrollView contentContainerStyle={styles.authContent} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.authIntro}><Text style={styles.authTitle}>{mode === "verify" ? "Verify your email" : mode === "forgot" ? "Forgot password?" : mode === "reset" ? "Create a new password" : "Welcome to CampusGig"}</Text><Text style={styles.authSubtitle}>{mode === "verify" ? `Enter the code sent to ${email}.` : mode === "forgot" ? "We’ll send a reset code to your account email." : mode === "reset" ? "Enter your code and choose a secure new password." : "Your trusted marketplace for verified student skills and services."}</Text></View>
        <View style={styles.authCard}>
          {(mode === "login" || mode === "register") && <View style={styles.authTabs} onLayout={(event) => setTabTrackWidth(event.nativeEvent.layout.width)}>
          ...
          <Animated.View style={{ opacity: formMotion, transform: [{ translateY: formMotion.interpolate({ inputRange: [0, 1], outputRange: [8, 0] }) }] }}>
            ...
            {mode === "register" && <Pressable onPress={() => setIsStudent((value) => !value)} style={({pressed})=>[styles.studentChoice,isStudent&&styles.studentChoiceActive,pressed&&styles.pressed]}><View style={[styles.studentCheckbox,isStudent&&styles.studentCheckboxActive]}><Text style={styles.studentCheckboxText}>{isStudent ? "✓" : ""}</Text></View><View style={styles.studentChoiceCopy}><Text style={styles.studentChoiceTitle}>I’m currently a student</Text><Text style={styles.studentChoiceBody}>Select for school verification and future provider access.</Text></View></Pressable>}
            ...
            <View style={styles.authTrust}><Text style={styles.authTrustIcon}>✓</Text><Text style={styles.authTrustText}>Personal email is accepted. School email is optional and account details stay private.</Text></View>
          </Animated.View>
        </View>
...
function AccountSettings({ user }: { user: AuthUser }) {
  ...
  return <View style={styles.expandPanel}>
    <Text style={styles.formTitle}>Account and security</Text><Text style={styles.formHelp}>Your account can hold more than one role as you grow on CampusGig.</Text>
    <View style={styles.accountDetail}><Text style={styles.accountDetailLabel}>EMAIL</Text><Text style={styles.accountDetailValue}>{user.email}</Text></View>
    <View style={styles.accountDetail}><Text style={styles.accountDetailLabel}>ACTIVE ROLES</Text><View style={styles.roleList}>{user.roles.map((role) => <View key={role} style={styles.miniRolePill}><Text style={styles.miniRoleText}>{role}</Text></View>)}</View></View>
    <Text style={styles.panelSectionTitle}>CHANGE PASSWORD</Text>
    <Text style={styles.inputLabel}>CURRENT PASSWORD</Text><TextInput value={currentPassword} onChangeText={setCurrentPassword} secureTextEntry placeholder="Enter current password" placeholderTextColor="#929A96" style={styles.profileInput}/>
    <Text style={styles.inputLabel}>NEW PASSWORD</Text><TextInput value={newPassword} onChangeText={setNewPassword} secureTextEntry placeholder="At least 8 characters" placeholderTextColor="#929A96" style={styles.profileInput}/>
    <Text style={styles.inputLabel}>CONFIRM NEW PASSWORD</Text><TextInput value={confirmPassword} onChangeText={setConfirmPassword} secureTextEntry placeholder="Enter new password again" placeholderTextColor="#929A96" style={styles.profileInput}/>
...
function Profile({ user, schools, avatarVersion, onAvatarChanged, onLogout }: { user: AuthUser; schools: School[]; avatarVersion: number; onAvatarChanged: () => void; onLogout: () => void }) {
  ...
  return <ScrollView style={styles.screen} contentContainerStyle={styles.standardContent} keyboardShouldPersistTaps="handled">
    <Text style={styles.eyebrow}>YOUR CAMPUSGIG</Text><Text style={styles.pageTitle}>Profile</Text>
    ...
    <View style={styles.profileForm}>
      <Text style={styles.formTitle}>Set up your campus identity</Text><Text style={styles.formHelp}>This information connects you to the correct participating school.</Text>
      ...
      <Text style={styles.inputLabel}>PROGRAM / COURSE</Text><TextInput value={program} onChangeText={setProgram} placeholder="e.g. BS Information Technology" placeholderTextColor="#929A96" style={styles.profileInput}/>
      <Text style={styles.inputLabel}>YEAR LEVEL</Text><TextInput value={yearLevel} onChangeText={(value) => setYearLevel(value.replace(/[^0-9]/g,""))} placeholder="e.g. 3" placeholderTextColor="#929A96" keyboardType="number-pad" maxLength={2} style={styles.profileInput}/>
      <Text style={styles.inputLabel}>SHORT BIO (OPTIONAL)</Text><TextInput value={bio} onChangeText={setBio} placeholder="Tell the campus community about yourself" placeholderTextColor="#929A96" multiline maxLength={500} style={[styles.profileInput,styles.bioInput]}/>
    ...
    <View style={styles.verificationCard}>
      <Text style={styles.formTitle}>Verify your student identity</Text><Text style={styles.formHelp}>Upload a clear student ID. It stays private and is available only to authorized CampusGig administrators.</Text>
    ...
    {activePanel === "settings"&&<AccountSettings user={user}/>}
